...
Windows 桌面软件：按主题、正文或附件名中的关键词自动保存正文和附件，支持多个邮箱、独立规则目录和检查频率。
...
3. **新增规则**：选中邮箱 →“新增规则”，名称填“工程实践报告”，关键词填“工程实践”。多个关键词可用中文或英文逗号分隔；需要全部命中时勾选“要求全部关键词匹配”。
4. **选择保存方式**：为这条规则指定下载目录，设置检查频率（例如 5 分钟）和收件起止日期（结束日期可不限），勾选需要的正文、附件和原始邮件。仅收集文件时，取消自动回复；需要回复时先修改回复内容。保存规则。
5. **开始接收**：核对规则时间范围（默认最近 30 天起），点击主界面的“保存并开始”。新邮件会按规则处理，可在“归档记录”查看结果并打开目录。
...
当前版本 **1.7.0**。安装包尚未数字签名，Windows 可能显示安全提示。QQ 云附件只记录链接，需要手动下载；Outlook 的 OAuth 登录暂未支持。自动化测试和界面检查已通过，真实邮箱收发仍需使用者验证。
...
二值选项统一使用开关，蓝色开启、灰色关闭。支持还原带有特征标记的 GB 编码主题乱码；已下载的旧文件夹不会自动改名。

## 附件整理
默认保留附件原名：ZIP、7Z、RAR 放入下载目录，普通文档按邮件主题归组。相同内容不重复保存，同名不同内容追加短编号。邮件及元数据存入“收件记录”。现有旧目录不自动迁移或删除。
整封邮件默认上限为 150 MB；旧版默认 30 MB 自动调整为 150 MB，其他自定义值保留。附件上限仍按规则设置。运行日志显示关键词、时间范围等跳过原因；云附件仍需通过记录的链接下载。
...
Gitee 授权、源码同步和校验文件上传已验证；大附件使用流式上传并显示传输进度，上传后仍逐个校验；首次大附件同步尚待成功验证。
