...
internal sealed class RuleEditor : EditorForm
{
    ...
    public RuleEditor(MailRule? old=null,bool fromTemplate=false):base(fromTemplate?"从模板新增规则":old is null?"新增规则 · 关键词与目录成组保存":"编辑规则")
    {
        ...
        makeInstructions.Click+=(_,_)=>{try{using var dialog=new SubjectInstructionsForm(instructionTopic.Text,Read());dialog.ShowDialog(this);}catch(Exception e){MessageBox.Show(this,e.Message,"请完善规则");}};
