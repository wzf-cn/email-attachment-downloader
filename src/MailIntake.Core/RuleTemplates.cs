...
public static class RuleTemplates
{
    ...
    public static MailRule Load(string path)
    {
        ...
        if(rule.Mode is not ("关键词" or "结构校验")||rule.KeyMode is not ("名单" or "固定秘钥")||rule.Fields is null||rule.Keywords is null||rule.MaxAttachmentMb<1||rule.MaxAttachmentMb>500)
