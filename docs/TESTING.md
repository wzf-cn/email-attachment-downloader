...
覆盖：主题结构；名单姓名与学号；CSV 前导零；目录必填；Unicode 附件归档与元数据；多目录归档单次回复；五次错误及第六次停收通知；跨账号计数；重启持久化；管理员按邮箱重置；旧邮件不补处理；发送不确定时不重试；自动邮件防循环；全局回复限额；大邮件不读取正文；同主题跨邮箱差异提醒。
...
3. 错误主题前五次统一回复，第六次通知停收；其后正确主题也不处理。
